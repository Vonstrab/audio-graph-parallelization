...
Cette configuration se veut proche des cas rencontrées dans la vie
réelle. 
\begin{figure}[H]
\begin{centering}
...
aux trois autres méthodes. Dans cette configuration on voit que les
4 c½urs ne font pas perdre tant de performances que ça, juste un légère
augmentation du nombre de deadlines rates.
...
Dans cette série de mesures on va fier le buffer a 128, le nombre
de threads a 3 et on va comparer l'éxécution entre la machine au repos
et la machine qui lit une vidéo en haute qualité.
...
Le cas le plus intéressant est celui du Râteau ou on voit que lors
de la lecture de la vidéo, toutes les méthodes parallèles ont plus
de mal a tenir les deadlines, contrairement a l\textquoteright exécution
séquentielle qui elle tient la ligne avec la surcharge, on analyser
cette dichotomie par le fait que la lecture d'une vidéo va monopoliser
un partie non négligeable des ressources de la machine ce qui néglige
une partie des gain de la parallélisation.
...
visible notamment dans l'histogramme du losange et du râteau, cette
méthode peut être prometteuse surtout avec un soin porté a son implémentation.
